#!/usr/bin/env bash
# FP16 baseline multimodal evaluation for Qwen2.5-VL-7B.
#
# Suite: mmmu_val / realworldqa / ai2d (tokens=16) + ocrbench (tokens=128)
#
# Writes:
#   - outputs_qwen25vl/<run>/...
#   - log/qwen25vl_7b/fp16/run_*.log, status_*.txt, summary_*.{txt,csv}
#
# Usage:
#   bash scripts/run_qwen25vl7b_fp16.sh
#   SMOKE=1 bash scripts/run_qwen25vl7b_fp16.sh
...
LOG_DIR="${ROOT_DIR}/log/qwen25vl_7b/fp16"
...
echo "========================================="
echo "Qwen2.5-VL-7B FP16 eval  ${STAMP}"
...
{
  echo "Qwen2.5-VL-7B FP16 summary  ${STAMP}"
  echo "MODEL_PATH=${MODEL_PATH}"
  echo "OUTPUT_ROOT=${OUTPUT_ROOT}"
  echo ""
  echo "==== Job status ===="
...
echo "========================================="
echo "Qwen2.5-VL-7B FP16 finished."
